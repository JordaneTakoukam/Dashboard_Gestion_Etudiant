...
export function CustomDropDown({ items, title, defaultValue, initialSelection, onSelect, alignTop = true, paddingY = 'py-0.5 lg:py-2' }: CustomDropDownProps) {
    const [selected, setSelected] = useState(initialSelection || defaultValue);
    const [updatedItems, setUpdatedItems] = useState<string[]>(items);
    ...
    dark:border-form-strokedark dark:bg-meta-4 dark:text-white dark:focus:border-primary flex justify-between items-center 
       pl-2  lg:pl-6 gap-x-1 lg:gap-x-3 bg-white  ${paddingY}  px-2 lg:px-4.5 
         ...
         `}>
                        <span className="block truncate text-[11px] lg:text-sm">{alignTop && selected}</span>
                            ...
                            {updatedItems.map((item, index) => (
                                ...
                                >
                                    {({ selected }) => (
                                        <>
                                            <span
                                                className={`block truncate text-[11px] lg:text-sm ${selected  && alignTop ? 'font-bold' : 'font-normal'
                                                    }`}
                                            >
                                                {item}
                                            </span>
