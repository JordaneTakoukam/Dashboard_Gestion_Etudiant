...
export function ModalCreateUpdateAdmin({ admin }: ModalCreateUpdateAdmin) {
    ...
    useEffect(() => {
        ...
        if (admin) {
            ...
            setMatricule(admin.matricule ? admin.matricule.toString() : "");
            setNom(admin.nom);
            setPrenom(admin.prenom);
            setGenre(admin.genre);
            setDateNaiss(admin.date_naiss ? admin.date_naiss.toString() : "");
            setLieuNaiss(admin.lieu_naiss);
            setEmail(admin.email);
            setContact(admin.contact);

            const currentGrade = services.find(grade => (grade._id === admin.grade));
            setGrade(currentGrade);

            const currentCategorie = services.find(categorie => (categorie._id === admin.categorie));
            setCategorie(currentCategorie);

            const currentFonction = services.find(fonction => (fonction._id === admin.fonction));
            setFonction(currentFonction);

            const currentSerivce = services.find(service => (service._id === admin.service));
            setService(currentSerivce);

            const currentRegion = regions.find(region => (region._id === admin.region));
            setRegion(currentRegion);

            const currentDepartement = departements.find(departement => (departement._id === admin.departement));
