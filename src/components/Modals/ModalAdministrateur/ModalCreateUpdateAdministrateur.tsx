...
function ModalCreateUpdateAdmin({ admin }: { admin: AdminType | null }) {
    ...
    useEffect(() => {
        if (admin) {
            setModalTitle(t('form_update.enregistrer') + t('form_update.admin'));
            // const currentNiveau = niveaux.find(niveau => niveau._id === "" + admin.niveaux[0].niveau);
            ...
            const currentCommune = communes.find(commune => commune._id === "" + admin.commune);
            ...
            const currentCategorie = categories.find(categorie => categorie._id === "" + admin.categorie);
            ...
            setNom(admin.nom);
            setPrenom(admin.prenom ? admin.prenom : ""); setGenre(admin.genre);
            setDateNaiss(admin.date_naiss ? admin.date_naiss.split("T")[0] : "");
            setLieuNaiss(admin.lieu_naiss ? admin.lieu_naiss : "");
            ...
            setFonction(admin.fonction ? fonctions.find(fonction => fonction._id === admin.fonction) : undefined);
            setService(admin.service ? services.find(service => service._id === admin.service) : undefined);
            setRegion(currentRegion);
            setDepartement(currentDepartement);
            setCommune(currentCommune);
            setDateEntreeAdmin(admin.date_entree ? admin.date_entree.toString() : "");
        } else {
            setModalTitle(t('form_save.enregistrer') + t('form_save.admin'));
            setNom("");
            setPrenom("");
            setGenre("");
            setDateNaiss("");
...
export default ModalCreateUpdateAdmin;
