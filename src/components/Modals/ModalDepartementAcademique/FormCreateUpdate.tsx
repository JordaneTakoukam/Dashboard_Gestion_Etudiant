...
import { apiCreateDepartementAcademique, apiUpdateDepartementAcademique } from '../../../api/settings/api_departement_academique';


function ModalCreateUpdate({ departementAcademique }: { departementAcademique: CommonSettingProps | null }) {
    ...
    useEffect(() => {
        if (departementAcademique) {
            setModalTitle(t('form_update.enregistrer') + t('form_update.departementAcademique'));
            setCode(departementAcademique.code);
            setLibelleFr(departementAcademique.libelleFr);
            setLibelleEn(departementAcademique.libelleEn);

        } else {
            setModalTitle(t('form_save.enregistrer') + t('form_save.departementAcademique'));
            setCode("");
            setLibelleFr("");
            setLibelleEn("");
        }
    ...
    }, [departementAcademique, isFirstRender, t]);
    ...
    const handleCreateUpdate = async () => {
                ...
                await apiCreateDepartementAcademique(
                    ...
                    if (e.success) {
                        ...
                        dispatch(createSettingItem({
                            tableName: 'departementsAcademique', newItem: {
                                code: e.data.code,
        ...
        //update
            ...
            if (!libelleFr || !libelleEn) {
                ...
                //
                // mise a jour
                await apiUpdateDepartementAcademique(
                    { _id: departementAcademique._id, code, libelleFr, libelleEn }
                ).then((e: ReponseApiPros) => {
                    if (e.success) {
                        createToast(e.message[lang as keyof typeof e.message], '', 0);
                        dispatch(updateSettingItem({
                            tableName: 'departementsAcademique',
                            updatedItem: {
                                code: e.data.code,
                                libelleFr: e.data.libelleFr,
                                libelleEn: e.data.libelleEn,
