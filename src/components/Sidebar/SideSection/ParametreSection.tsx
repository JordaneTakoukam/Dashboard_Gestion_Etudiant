...
const ParametreSidebarLink = ({
    ...
    const menuItems = [
        ...
        { permission: hasAdminPermission, path: '/parametres/admins', label: t('sub_menu.administrateurs') },
        { permission: hasQrCodePermission, path: '/parametres/qr-code', label: t('sub_menu.qr_code') },
        { permission: hasServicePermission, path: '/parametres/services', label: t('sub_menu.services') },
        { permission: hasFunctionPermission, path: '/parametres/fonctions', label: t('sub_menu.fonctions') },
        { permission: hasGradePermission, path: '/parametres/grades', label: t('sub_menu.grades') },
        { permission: hasCategoryPermission, path: '/parametres/categories', label: t('sub_menu.categories') },
                ...
                {(handleClick, open) => (
                    <>
                        <NavLink
                            ...
                            className={`group relative flex items-center gap-2.5 rounded-sm py-2 pl-3 pr-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${pathname.includes('parametres') ? 'bg-graydark dark:bg-meta-4 text-secondary' : ''}`}
                        >
