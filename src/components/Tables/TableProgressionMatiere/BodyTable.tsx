...
const BodyTable = ({ data }: { data: MatiereType | undefined }) => {
    ...
    const handleCheckboxChange = async (objectifIndex: number) => {
        if (roles.admin === userRole || roles.superAdmin === userRole || roles.enseignant === userRole) {
            if (matiereData && matiereData.objectifs) {
                const updatedObjectifs = matiereData.objectifs.map((objectif, idx) => {
                    if (idx === objectifIndex) {
                        ...
                        apiUpdateObjectif({
                            ...
                            if (response.success) {
                                ...
                                if (matiereData._id && matiereData.objectifs) {
                                    dispatch(
                                        updateMatiere({
                                            id: matiereData._id,
                                            matiereData: {
    ...
    return (
        <tbody>
            {matiereData && matiereData.objectifs && matiereData.objectifs.map((objectif: ObjectifType, indexObjectif: number) => (
