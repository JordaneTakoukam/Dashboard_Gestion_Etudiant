            ...
            {data.map((item, index) => (
                <tr key={index + 1} className="font-medium text-black dark:text-white text-[12px] md:text-[14px]">
                    ...
                    <td className="border-b border-[#eee] py-0 lg:py-4 px-4 dark:border-strokedark bg-gray-2 dark:bg-black hidden md:table-cell">
                        <h5>
                            {lang === 'en'
                                ? `From ${formatDate2(item.date_debut_absence)} to ${formatDate2(item.date_fin_absence)}`
                                : `From ${formatDate2(item.date_debut_absence)} at ${formatDate2(item.date_fin_absence)}`}
                        </h5>
                    </td>
